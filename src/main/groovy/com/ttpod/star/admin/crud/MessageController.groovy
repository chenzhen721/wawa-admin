...
import com.ttpod.star.admin.PushController
...
class MessageController extends BaseController{
    ...
    def messages(){adminMongo.getCollection("messages")}
    def push_msgs(){adminMongo.getCollection("push_msgs")}
    def sys_push(){adminMongo.getCollection("sys_push")}

    def msgs(){mainMongo.getCollection("msgs")}

    def msgs_global(){mainMongo.getCollection("msgs_global")}
        ...
        if(msg.get("s_type") == 0){//发送方式 用户ID
            ...
            while(allPage > 0){
                ...
                while(count > 0 && index++ <= 5000){
                    ...
                    user_ids = users().find(query, $$(_id ,1)).sort($$(_id:1)).limit(step).toArray()*._id;
...
//        Integer platform = req['platform'] as Integer ?: 0
        ...
        String userIds = req['userIds']?.trim();
