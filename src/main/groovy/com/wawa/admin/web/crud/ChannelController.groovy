...
class ChannelController extends BaseController {
    ...
    def add_properties(HttpServletRequest req) {
        Map properties = req.getParameterMap() as Map<String, String[]>
    ...
    def static_channels(HttpServletRequest req) {
        ...
        adminMongo.getCollection('channels').find(query).toArray().each { DBObject channnel ->
            ...
            def regUsers = users.find(user_query, new BasicDBObject('status', 1)).toArray().collect{it.get(_id)}
