...
import com.wawa.service.play.Qiyiguo
import com.wawa.service.play.dto.QiygGoodsDTO
...
class CatchuController extends BaseController {
    ...
    def success_record_refuse(HttpServletRequest req) {
        ...
        if (post_log != null) {
            ...
            def toys = post_log['toys'] as List<DBObject>
            if (toys != null && toys.size() > 0) {
                toys.each { DBObject toy ->
    ...
    def post_unbox(HttpServletRequest req) {
        ...
        if (post_log != null) {
            def toys = post_log['record_ids'] as List<String>
            if (toys != null && toys.size() > 0) {
    ...
    def pull_order(HttpServletRequest req, HttpServletResponse res) {
        ...
        apply_post_logs().find(query).toArray().each {DBObject obj ->
            logger.info('===============>:' + obj['_id'])
    ...
    def push_order(HttpServletRequest req) {
        ...
        def missing = []
        def error = []
        apply_post_logs().find(query).limit(50).toArray().each {DBObject obj ->
            def set = new BasicDBObject()
            def inc = [n: 1]
